...
import { MultiApiKeyManager } from './components/MultiApiKeyManager';
...
import { generateIllustratedCards, generateTextToImage, generateComicStrip, generateVideoScriptsForComicStrip, generateVideo, getVideosOperation, generateVideoTransition } from './services/enhancedGeminiService';
...
import { MultiApiKeyService, type ApiKeyEntry } from './services/multiApiKeyService';
...
const App: React.FC = () => {
  ...
  const [isMultiApiKeyManagerOpen, setIsMultiApiKeyManagerOpen] = useState<boolean>(false);
  const [multiApiKeys, setMultiApiKeys] = useState<ApiKeyEntry[]>([]);
  ...
  useEffect(() => {
    ...
    if (envApiKey) {
      ...
      // 检查多API密钥模式
      const multiKeys = MultiApiKeyService.getAllApiKeys();
      if (multiKeys.length > 0) {
        setMultiApiKeys(multiKeys);
        // 获取当前活动的API密钥
        const activeKey = MultiApiKeyService.getActiveApiKey();
        if (activeKey) {
          setApiKey(activeKey);
        }
      } else {
        // 如果没有多API密钥，则检查单个API密钥
  ...
  const handleGenerateWiki = async () => {
    ...
    try {
      const imageUrls = await generateIllustratedCards(wikiPrompt, activeStyle, wikiModel);
  ...
  const handleGenerateComicStrip = async () => {
    ...
    try {
        const { imageUrls, panelPrompts } = await generateComicStrip(comicStripStory, comicStripStyle, comicStripNumImages);
        ...
        for (let i = 0; i < comicStripImages.length; i += concurrencyLimit) {
            ...
            await Promise.all(chunk.map(async (image, chunkIndex) => {
                ...
                try {
                    ...
                    let operation = await generateVideo(script, file, '16:9', 'subtle');
            ...
            for (let i = 0; i < numTransitions; i += transitionConcurrencyLimit) {
                ...
                await Promise.all(chunkIndices.map(async (index) => {
                    ...
                    try {
                        ...
                        let operation = await generateVideoTransition(startImage, nextSceneScript, comicStripStory, comicStripStyle);
...
const handleComicPanelEditComplete = async (index: number, newImageSrc: string, newPrompt: string) => {
  ...
  const handleGenerateTextToImage = async () => {
    ...
    try {
        ...
        const imageUrls = await generateTextToImage(finalPrompt, textToImageNegativePrompt, textToImageNumImages, textToImageAspectRatio);
        const resizedImageUrls = await Promise.all(imageUrls.map(src => resizeImage(src, 800)));
        const imagesWithIds: GeneratedImage[] = resizedImageUrls.map((src, index) => ({
            id: `${Date.now()}-${index}`,
            src,
  ...
  const handleClosePreview = () => {
   ...
   const handleApiKeySave = (newKey: string) => {
    if (newKey.trim()) {
      ...
      if (!isEnvApiKey) {
        // 如果已经有多API密钥，则更新第一个密钥
        if (multiApiKeys.length > 0) {
          const updatedKeys = [...multiApiKeys];
          updatedKeys[0] = {
            ...updatedKeys[0],
            key: trimmedKey,
            name: updatedKeys[0].name || '默认 API Key'
          };
          setMultiApiKeys(updatedKeys);
          MultiApiKeyService.saveApiKeys(updatedKeys);
        } else {
          // 否则创建一个新的API密钥条目
          ...
          const newKeys = [newApiKeyEntry];
          setMultiApiKeys(newKeys);
          MultiApiKeyService.saveApiKeys(newKeys);
        }
  ...
  const handleMultiApiKeySave = (apiKeys: ApiKeyEntry[]) => {
    // 保存多API密钥
    setMultiApiKeys(apiKeys);
    MultiApiKeyService.saveApiKeys(apiKeys);
    
    // 如果有API密钥，设置第一个为当前活动密钥
    if (apiKeys.length > 0 && apiKeys[0].key) {
      setApiKey(apiKeys[0].key);
    }
    
    setIsMultiApiKeyManagerOpen(false);
    setError(null);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      <Header 
        ...
        onMultiApiKeyClick={isEnvApiKey ? undefined : () => setIsMultiApiKeyManagerOpen(true)} // 只有多API密钥管理按钮
        appMode={appMode}
      ...
      <MultiApiKeyManager
        ...
        onSave={handleMultiApiKeySave}
        currentApiKeys={multiApiKeys}
      />
    </div>
  );
};
